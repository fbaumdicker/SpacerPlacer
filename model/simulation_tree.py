...
from model.helpers.raw_data_tools import Species as Species
...
class SimulationTree(AdvancedTree):
    ...
    def __init__(self, sim_deletion_model, event_sim, gtr_gain_rate, gtr_loss_rate, deletion_model_parameter,
        ...
        self.eps = 1e-12
        self.big_eps = 1e10  # look at values in treetime ttconf.big_number/tiny_number
    ...
    def get_relative_loss_positions_branch_based(self):
        ...
        for c in self.root.find_clades():
            ...
            ls_losses.append(losses)
            ls_nb_ex_spacers.append(nb_ex_spacers)
        # print('relative loss pos', dict_relative_pos)
    ...
    def get_branch_based_fes_les_pos(self):
        ...
        for c in self.root.find_clades():
            ...
            # print(c.name)
            # print(fes_m_presence)
            # print(les_m_presence)
            # print(nb_ex_spacers)
        ...
        # print('relative loss pos', dict_fes_les)
        return dict_fes_les
    ...
    def get_relative_loss_stats(self):
        ...
        for c in self.root.find_clades():
            ...
            # print(fes_les_l_s_m_presence[2])
            ...
            # print(global_fes_les_l_s_m_presence)
    ...
    def sample_loss_events_blm(self, c):
        ...
        while i < n:
            ...
            # lost_spacers_asstr = '(' + ', '.join([str(int(a)) for a in c.up.spacers[i:i + fragment_length]]) + ')'
            ...
            while t_c < bl:
                ...
                if t_c > bl:
                    ...
                    node.gain_loss_events.append(('-', t_c, lost_spacers_asstr, len(ex_spacers), ls_loss_array_pos,
                                                  spacers))
                    self.gain_loss_events_dict[node.name].append(node.gain_loss_events[-1])
                    # print(node.name, lost_spacers_asstr, loss_array_pos)
                    if not lost_spacers_asstr:
    ...
    def visualize_tree(self, name='default', do_show=True, path='pictures', sort_top_order='nb_leafs',
        ...
        # if self.fragment_loss_model is not None:
        #     ls_frag_losses = []
        ...
        for node in node_iterator:
            ...
            # print('gain dict', self.gain_dict[node.name])
            # print('loss dict', self.loss_dict[node.name])
            # print('block loss dict', sim_block_losses)
            # print('df_changes', df_changes)
        ...
        # name_top_order = {i: s for i, s in enumerate(top_order)}
        ...
        # alignment = list(reversed(range(1, self.next_spacer)))
        # for node in self.root.find_clades(order='preorder'):
        #     df.loc[node.name] = [model_tools.spacer_array_to_binary(node.spacers,
        #                                                             alignment=alignment)]
        #     sim_block_losses = self.block_loss_dict.get(node.name, [])
        #     df_changes.loc[node.name] = [self.gain_dict[node.name],
        #                                  self.loss_dict[node.name],
